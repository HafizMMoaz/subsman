...
while ($userToNotify = $usersToNotify->fetchArray(SQLITE3_ASSOC)) {
    ...
    if ($row = $result->fetchArray(SQLITE3_ASSOC)) {
        ...
        $email['fromEmail'] = $row["from_email"] ? $row["from_email"] : "noreply@subsman.app";
        ...
        if (!empty($notify)) {
            ...
            if ($emailNotificationsEnabled) {
                ...
                foreach ($notify as $userId => $perUser) {
                    ...
                    $mail->setFrom($email['fromEmail'], 'Subsman');
                    ...
                    $mail->Subject = 'Subsman Notification';
                    $mail->Body = $message;
            ...
            if ($discordNotificationsEnabled) {
                ...
                if (!$ssrf) {
                    ...
                    foreach ($notify as $userId => $perUser) {
                        ...
                        $title = translate('subsman_notification', $i18n);
