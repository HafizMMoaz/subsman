...
if ($rows) {
    if ($admin['smtp_address'] && $admin['smtp_port'] && $admin['smtp_username'] && $admin['smtp_password'] && $admin['encryption']) {
        ...
        $fromEmail = empty($admin['from_email']) ? 'noreply@subsman.app' : $admin['from_email'];
        ...
        try {
            foreach ($rows as $user) {
                ...
                $mail->isHTML(true);
                $mail->Subject = 'Subsman - Reset Password';
                $mail->Body = '<img src="' . $server_url . '/images/siteicons/subsman.png" alt="Logo" />
                    <br>
                    A password reset was requested for your Subsman account.
                    <br>
